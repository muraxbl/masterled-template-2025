    ...
    <div class="block-promo">
      <div class="cart-voucher">
                    ...
                    {if $voucher.reduction_percent != '0.00'}
                        {foreach from=$cart.subtotals item="subtotal"}
                            {if $subtotal.type == 'products'}
                              {if isset($resta)}
                                  {math equation='x - y' x=$subtotal.amount y=$resta assign=total_productos}
                              {else}
                                  {assign var=total_productos value=$subtotal.amount}
                              {/if}
                            {break}
                            {/if}
                        {/foreach}
                          {$cupon = $total_productos * ($voucher.reduction_percent / 100)|floatval}
                          {$cupon|number_format:2:",":""}€                        
                    {else}
                        {if $voucher.reduction_amount != '0.00'}
        ...
        {if $customer.is_logged}
          {if in_array($customer.id_default_group, ['30','33','39'])}
          {else}
